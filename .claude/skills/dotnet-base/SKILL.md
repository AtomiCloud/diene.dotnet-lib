...
| `pls test` (`:unit` / `:int`)                     | Run all / one test suite                                      |
| `pls test:coverage` (`:unit:` / `:int:` variants) | Run tests with coverage and enforce the threshold             |
| `pls test:watch`                                  | Watch the fast unit suite                                     |
| `pls dead-code`                                   | Dead-code inspection (all projects + production-only pass)    |
...
- **Dead-code** — `scripts/local/dotnet-dead-code.sh` runs JetBrains InspectCode on net10 via the repo-pinned `jb` tool; the production-only pass surfaces production code reachable only from tests.
